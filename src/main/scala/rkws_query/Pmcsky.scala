...
class PmcSky(c_queries: ListBuffer[Query], c_slide: Int) {
  ...
  def process(elements: Dataset[(Int, Data_mcsky)], windowEnd: Long, spark: SparkSession): Unit = {
            ...
            if (output_slide.nonEmpty) {
              ...
              do {
                if (p._2.arrival >= window - W_distinct_list(w)) {
                  var i, y: Int = 0
                  var count = p._2.lSky.getOrElse(i + 1, ListBuffer()).count(_._2 >= window - W_distinct_list(w))
                  do {
                    if (count >= k_distinct_list(y)) { //inlier for all i
                      ...
                      i += 1
                      count += p._2.lSky.getOrElse(i + 1, ListBuffer()).count(_._2 >= window - W_distinct_list(w))
                    }
    ...
    inputList
      .filter(p => p._2.arrival < window - W_min + slide)
      .foreach(p => {
        //Remove small window points from clusters
        if(p._2.mc != -1 && p._2.arrival >= window - W_min)
          deleteSmallWindowPoint(p._2, current)
        //Remove old points
        if (p._2.arrival < window + slide)
  ...
  def checkPoint(el: Data_mcsky, window: Long, current: PmcskyState): Unit = {
    if (el.lSky.isEmpty && el.mc == -1) { //It's a new point!
      insertPoint(el, window, current)
    } else if (el.mc == -1) { //It's an old point
      updatePoint(el, window, current)
    }
  }

  def insertPoint(el: Data_mcsky, window: Long, current: PmcskyState): Unit = {
    val small_window = if( el.arrival >= window - W_min) true else false
        ...
        if (p.id != el.id) {
          if (closeMCs.keySet.contains(p.mc) || p.mc == -1) { //check only the points in PD and close MCs
            ...
            if (thisDistance <= R_max) {
              ...
              if (p.arrival >= window - W_min && p.mc == -1 && thisDistance <= R_min / 2) NC += p
            }
          }
        }
        res
    ...
    if (res)
      old_sky.foreach(l => { //Check the old skyband inputList
        ...
        if (thisDistance <= R_max) {
          val skyRes = neighborSkyband(el, p, thisDistance)
          if (!skyRes && thisDistance <= R_min) res = false
          if (p.arrival >= window - W_min && current.PD.contains(p.id) && thisDistance <= R_min / 2) NC += p
        }
      })
